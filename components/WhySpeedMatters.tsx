...
import { SATELLITES_PROGRAMS } from '../constants';
...
const ProgramCard: React.FC<{ program: Program }> = ({ program }) => (
    <div className="bg-white p-8 rounded-xl shadow-md transition-all duration-300 hover:shadow-lg hover:-translate-y-1 border-l-4 border-brand-space">
        <div className="flex items-center gap-4 mb-4">
            ...
            <span className="font-bold bg-blue-100 text-brand-space px-2 py-0.5 rounded-full text-sm">{program.ageGroup}</span>
        </div>
...
export const SatellitesPage: React.FC<PageProps> = ({ navigate }) => {
  return (
    <section id="satellites" className="py-16 md:py-24 animate-fade-in-up">
      <div className="container mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="flex justify-center lg:order-2">
                <img src="https://files.catbox.moe/r27c2u.png" alt="Cartoon Satellite" className="w-full max-w-md" />
            </div>
            <div className="text-center lg:text-left lg:order-1">
                <h2 className="text-5xl md:text-6xl font-bold tracking-tight text-brand-space">
        ...
        <div className="mt-16">
            <h3 className="text-2xl font-bold text-center text-slate-800 mb-8">Available Programs</h3>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                {SATELLITES_PROGRAMS.map((program, index) => (
                    <ProgramCard key={index} program={program} />
                ))}
            </div>
            <div className="mt-16 text-center bg-white p-10 rounded-2xl shadow-xl max-w-4xl mx-auto border-t-4 border-brand-space">
