...
import { ASTRONOMY_PROGRAMS } from '../constants';
...
const ProgramCard: React.FC<{ program: Program }> = ({ program }) => (
    <div className="bg-white p-8 rounded-xl shadow-md transition-all duration-300 hover:shadow-lg hover:-translate-y-1 border-l-4 border-indigo-500">
        <div className="flex items-center gap-4 mb-4">
            ...
            <span className="font-bold bg-indigo-100 text-indigo-600 px-2 py-0.5 rounded-full text-sm">{program.ageGroup}</span>
        </div>
        ...
        <div className="mt-16">
            <h3 className="text-2xl font-bold text-center text-slate-800 mb-8">Available Programs</h3>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                {ASTRONOMY_PROGRAMS.map((program, index) => (
